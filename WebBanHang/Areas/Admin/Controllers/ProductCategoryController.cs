        ...
        public ActionResult Create(ProductCategory model)
        {
            if(ModelState.IsValid)
            {
                try
                {
                    ...
                    {
                        SetAlert("Thêm danh mục thành công", "Thành công");
                    }
                    else
                    {
                        SetAlert("Added category successfully", "Success");
                    }
        ...
        public ActionResult Edit(ProductCategory model)
        {
            if (ModelState.IsValid)
            {
                try
                {
                    ...
                    model.ModifiedDate = DateTime.Now;
                    model.Alias = Filterchar.FilterChar(model.Title);
                    db.ProductCategorys.Attach(model);
                    db.Entry(model).State = System.Data.Entity.EntityState.Modified;
                    db.SaveChanges();
                    if(currentCulture == "vi")
                    {
                        SetAlert("Sửa danh mục thành công", "Thành công");
                    }
                    else
                    {
                        SetAlert("Edited directory successfully", "Success");
                    }
        ...
        public ActionResult DeleteAll(string ids)
        {
            if (!string.IsNullOrEmpty(ids))
            {
                ...
                if (items != null && items.Any())
                {
                    ...
                    {
                        var obj = db.ProductCategorys.Find(Convert.ToInt32(item));
